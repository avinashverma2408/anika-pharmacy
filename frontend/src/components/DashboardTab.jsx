...
export default function DashboardTab() {
  ...
  const forecastChart = forecast
    ? getForecastChartData(forecast.upcomingSeason)
    : null;
  ...
  if (forecast && forecastChart) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const yLY =
        pTop + gHeight - (forecastChart.lastYear[i] / maxVal) * gHeight;
      const yEx =
        pTop + gHeight - (forecastChart.expected[i] / maxVal) * gHeight;
      points.push({
        x,
        yLY,
        yEx,
        valLY: forecastChart.lastYear[i],
        valEx: forecastChart.expected[i],
        label: forecastChart.labels[i],
      });
    }
  ...
  const expiringSoonItems = dashboardStats?.expiringSoon
    ? dashboardStats.expiringSoon.map((m) => ({
        medicine: m,
    ...
    : medicines
        ...
        .map((m) => ({
          medicine: m,
          daysLeft: calculateDaysDifference(simulatedDate, m.expiryDate),
        }))
  ...
  const categoriesList = [
    ...
    percentage: Math.round(
      ((categoryCounts[cat] || 0) / totalActiveMeds) * 100,
    ),
  }));
  ...
  const getDynamicMonthlyBreakdown = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const matchedDbMonth = rawBreakdown.find(
        (entry) => entry.year === year && entry.month === monthNum,
      );
  ...
  const currentMonthVal = currentMonthData
    ? chartMetric === "revenue"
      ? currentMonthData.revenue
      : currentMonthData.profit
    : 0;
  const prevMonthVal = previousMonthData
    ? chartMetric === "revenue"
  ...
  const maxVal = Math.max(
    ...monthlyBreakdown.map((d) =>
      chartMetric === "revenue" ? d.revenue : d.profit,
    ),
    1,
  );
  ...
  const barGap =
    numBars > 1 ? (graphWidth - barWidth * numBars) / (numBars - 1) : 0;
  ...
  const avgProfitMargin =
    lifetimeRevenue > 0 ? (lifetimeProfit / lifetimeRevenue) * 100 : 0;
  ...
  if (trendLinePoints.length > 0) {
    ...
    splineAreaPath =
      splinePath +
      ` L ${trendLinePoints[trendLinePoints.length - 1].x} ${paddingTop + graphHeight} L ${trendLinePoints[0].x} ${paddingTop + graphHeight} Z`;
  }
    ...
    <section id="tab-dashboard" className="tab-pane active">
      <div className="page-header">
        ...
        <p className="subtitle">
          Quick overview of pharmacy products and upcoming medicine expiries.
        </p>
      </div>
      ...
      <div className="stats-grid">
        ...
        <div
          className="stat-card border-success"
          style={{ cursor: "pointer" }}
        ...
        <div
          className="stat-card border-danger"
          style={{ cursor: "pointer" }}
        ...
        <div
          className="stat-card border-orange"
          style={{ cursor: "pointer" }}
        ...
        <div
          className="stat-card border-inactive"
          style={{ cursor: "pointer" }}
      ...
      <div className="dashboard-analytics-section">
        ...
        <div className="analytics-card card-panel">
          ...
          >
            <div className="panel-title-group">
              ...
              >
                <i
                  className="fa-solid fa-chart-column panel-icon"
                  style={{
                    color:
                      chartMetric === "revenue" ? "var(--primary)" : "#10b981",
                  }}
                ></i>
                <h3 style={{ margin: 0 }}>
                  {chartMetric === "revenue"
              ...
              <div className="trend-summary-highlights">
                ...
                <span
                  className="trend-highlight-value"
                  style={{
                    color:
                      chartMetric === "revenue" ? "var(--primary)" : "#10b981",
                    fontWeight: "700",
                ...
                >
                  ₹
                  {currentMonthVal.toLocaleString("en-IN", {
                    minimumFractionDigits: 2,
                  })}
                </span>
                {prevMonthVal > 0 && (
                  <span
                    className={`trend-highlight-badge ${percentageChange >= 0 ? "up" : "down"}`}
                  >
                    <i
                      className={
                        percentageChange >= 0
                    ...
                    ></i>
                    {percentageChange >= 0 ? "+" : ""}
                    {percentageChange.toFixed(1)}% MoM
                  </span>
                )}
              </div>
            </div>
              ...
              <defs>
                ...
                <linearGradient id="trendLineGrad" x1="0" y1="0" x2="1" y2="0">
                  <stop
                    offset="0%"
                    stopColor={
                      chartMetric === "revenue" ? "#d946ef" : "#10b981"
                    }
                  />
                  <stop
                    offset="100%"
                    stopColor={
                      chartMetric === "revenue" ? "#06b6d4" : "#34d399"
                    }
                  />
                </linearGradient>
                ...
                >
                  <feDropShadow
                    ...
                    stdDeviation="4"
                    floodColor={
                      chartMetric === "revenue" ? "#ec4899" : "#10b981"
                    }
                    floodOpacity="0.4"
                  />
                </filter>
        ...
        <div className="analytics-card card-panel">
          <div className="analytics-card-header">
            <div className="panel-title-group">
              <i
                className="fa-solid fa-chart-pie panel-icon"
          ...
          <div className="category-distribution-list">
            {categoriesList.map((cat) => {
              ...
              if (cat.name === "Tablet") {
                ...
                glowColor = "#3b82f6";
              } else if (cat.name === "Capsule") {
                gradient = "linear-gradient(90deg, #d946ef 0%, #f472b6 100%)";
                glowColor = "#d946ef";
              } else if (cat.name === "Syrup") {
                gradient = "linear-gradient(90deg, #10b981 0%, #34d399 100%)";
                glowColor = "#10b981";
              } else if (cat.name === "Injection") {
                gradient = "linear-gradient(90deg, #ec4899 0%, #f472b6 100%)";
                glowColor = "#ec4899";
              } else if (cat.name === "Vaccine") {
                gradient = "linear-gradient(90deg, #8b5cf6 0%, #a78bfa 100%)";
                glowColor = "#8b5cf6";
              } else if (cat.name === "Ointment") {
          ...
          <div
            className="analytics-financial-summary"
            style={{ gridTemplateColumns: "repeat(3, 1fr)" }}
          >
            <div className="financial-card card-month">
              <div className="fin-card-header">
                <span className="fin-label">Current Month Sales</span>
                <div className="fin-icon-wrap bg-primary-glow">
                  <i className="fa-solid fa-calendar-days text-primary-color"></i>
                </div>
              </div>
              <h4 className="fin-value">
                ₹
            ...
            <div className="financial-card card-lifetime">
              <div className="fin-card-header">
                ...
                <div className="fin-icon-wrap bg-success-glow">
                  <i className="fa-solid fa-sack-dollar text-success-color"></i>
                </div>
              </div>
              <h4 className="fin-value">
                ₹
            ...
            <div className="financial-card card-invoices">
              <div className="fin-card-header">
                ...
                <div className="fin-icon-wrap bg-warning-glow">
                  <i className="fa-solid fa-file-invoice text-warning-color"></i>
                </div>
              </div>
            ...
            <div className="financial-card card-month-profit">
              <div className="fin-card-header">
                ...
                <div className="fin-icon-wrap bg-profit-glow">
                  <i className="fa-solid fa-money-bill-trend-up"></i>
                </div>
              </div>
              <h4 className="fin-value" style={{ color: "#10b981" }}>
                ₹
            ...
            <div className="financial-card card-lifetime-profit">
              <div className="fin-card-header">
                ...
                <div className="fin-icon-wrap bg-month-profit-glow">
                  <i className="fa-solid fa-vault"></i>
                </div>
              </div>
              <h4 className="fin-value" style={{ color: "#8b5cf6" }}>
                ₹
                {lifetimeProfit.toLocaleString("en-IN", {
                  minimumFractionDigits: 2,
                })}
              </h4>
            </div>
            <div className="financial-card card-margin">
              <div className="fin-card-header">
                ...
                <div className="fin-icon-wrap bg-margin-glow">
                  <i className="fa-solid fa-percent"></i>
                </div>
              </div>
              <h4 className="fin-value" style={{ color: "#f59e0b" }}>
      ...
      <div className="dashboard-split">
        ...
        <div className="split-left card-panel">
          <div className="panel-header">
            ...
            <span
              className={`badge ${expiringSoonCount > 0 ? "badge-warning" : "badge-safe"}`}
            >
              {expiringSoonCount > 0
              ...
              {expiringSoonItems.length === 0 ? (
                <div className="empty-state">
                  <i
                    ...
                    style={{ fontSize: "28px", color: "var(--success)" }}
                  ></i>
                    ...
                    >
                      ...
                      <div className="timeline-content">
                        ...
                        <div className="timeline-desc">
                          Batch No: {med.batch} | Qty: {med.quantity} |
                          Category: {med.category}
                        </div>
                        <div className="timeline-meta">
                          <span>
                            Expiry: {formatDateDisplay(med.expiryDate)}
                          </span>
        ...
        <div className="split-right">
          ...
          {forecast && (
            <div
              className="card-panel forecaster-card"
              style={{ borderLeft: `4px solid ${forecast.seasonColor}` }}
            >
              <div className="forecaster-header">
                <div
                  className="forecaster-title-group"
                  style={{ display: "flex", gap: "12px", alignItems: "center" }}
                >
                  <i
                    className={`fa-solid ${forecast.seasonIcon} forecaster-icon`}
                    style={{ color: forecast.seasonColor, fontSize: "22px" }}
                  ></i>
                  <div>
                    ...
                    <span
                      className="forecast-season-badge"
                      style={{
                        backgroundColor: `${forecast.seasonColor}22`,
                        color: forecast.seasonColor,
                        fontSize: "11px",
                        padding: "2px 8px",
                        borderRadius: "12px",
                        fontWeight: "600",
                    ...
                    >
                      Upcoming: {forecast.upcomingSeason} (
              ...
              <div className="forecaster-body" style={{ marginTop: "16px" }}>
                ...
                >
                  Current season is <strong>{forecast.currentSeason}</strong>.
                  Based on sales analytics, demand for{" "}
                  <strong>{forecast.primaryCategory}s</strong> will spike by{" "}
                  <span
                    style={{ color: forecast.seasonColor, fontWeight: "700" }}
                  >
                    {forecast.expectedIncrease}
                  </span>{" "}
                  during <strong>{forecast.upcomingSeason}</strong>.
                </p>

                <div
                  className="forecast-alert-box"
                  style={{
                ...
                >
                  ...
                  {forecast.restockNeeded > 0 ? (
                    ...
                    >
                      ...
                      <span>
                        <strong>Restock Suggestion:</strong> Add{" "}
                        {forecast.restockNeeded} units of{" "}
                        {forecast.primaryCategory}s.
                      </span>
                    </div>
                  ) : (
                    <div
                    ...
                    >
                      <i className="fa-solid fa-circle-check"></i>
                      <span>
                        <strong>Stock Adequate:</strong> No immediate restock
                        needed.
                      </span>
                    </div>
                  )}
                ...
                >
                      ...
                      >
                        <span
                          style={{
                            ...
                            width: "8px",
                            height: "2px",
                            backgroundColor: forecast.seasonColor,
                          }}
                        ></span>{" "}
                        Expected
                      </span>
                  ...
                  >
                    <defs>
                      <linearGradient
                        id={`forecastAreaGrad-${forecast.upcomingSeason}`}
                        x1="0"
                        y1="0"
                        x2="0"
                        y2="1"
                      >
                        <stop
                          offset="0%"
                          stopColor={forecast.seasonColor}
                          stopOpacity="0.3"
                        />
                        <stop
                          offset="100%"
                    ...
                    <path
                      d={expectedAreaPath}
                      fill={`url(#forecastAreaGrad-${forecast.upcomingSeason})`}
                    />
                    ...
                    <path
                      d={expectedPath}
                      fill="none"
                      stroke={forecast.seasonColor}
                      strokeWidth="2.25"
                      className="forecast-expected-path"
                    />
                ...
                <button
                  ...
                  style={{
                    marginTop: "16px",
                    borderColor: forecast.seasonColor,
                    color: forecast.seasonColor,
                    background: "transparent",
                  }}
                >
                  <i className="fa-solid fa-magnifying-glass-chart"></i> View{" "}
                  {forecast.primaryCategory} Stock
                </button>
              </div>
            </div>
          )}
          ...
          <div className="card-panel quick-actions-panel">
            ...
            <div className="actions-buttons-grid">
              ...
              >
                <i className="fa-solid fa-vial-circle-check"></i> Test
                Notification
              </button>
              <button
                className="btn btn-outline btn-icon"
                onClick={() => setActiveTab("notifications-log")}
              >
          ...
          <div className="info-card">
            ...
            <div className="info-text">
              ...
              <p>
                This application will alert you in real-time. Make sure to allow
                browser notification permissions for system tray desktop popups!
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="card-panel mt-4" style={{ marginTop: "24px" }}>
        <div className="panel-header">
          <div className="panel-title-group">
            <i
              className="fa-solid fa-ranking-star panel-icon text-primary"
        ...
        <div className="performance-grid">
          ...
          <div className="perf-sub-panel">
            <h4>
              <i
                className="fa-solid fa-fire text-orange"
            ...
            <div className="perf-list">
              {topSelling.length === 0 ? (
                ...
                >
                  <i
                    className="fa-solid fa-chart-bar text-muted"
                    style={{
                      fontSize: "24px",
                      opacity: 0.5,
                      marginBottom: "8px",
                ...
                topSelling.map((med, idx) => (
                  <div key={idx} className="perf-item">
                    ...
                    <div className="perf-stats">
                      ...
                      <div className="perf-subtext">
                        Sales: ₹{med.sales.toLocaleString("en-IN")}
                      </div>
                    </div>
                  </div>
                ))
          ...
          <div className="perf-sub-panel">
            <h4>
              <i
                className="fa-solid fa-gem text-success"
                style={{ color: "#10b981" }}
              ></i>{" "}
              Most Profitable Products (Net Profit Contribution)
            </h4>
            <div className="perf-list">
              {topProfitable.length === 0 ? (
                ...
                >
                  <i
                    className="fa-solid fa-sack-dollar text-muted"
                    style={{
                      fontSize: "24px",
                      opacity: 0.5,
                      marginBottom: "8px",
                ...
                topProfitable.map((med, idx) => (
                  <div key={idx} className="perf-item profit-hover">
                    <div className="perf-rank" style={{ color: "#10b981" }}>
                      #{idx + 1}
                    </div>
                    <div className="perf-details">
                      <div className="perf-name">{med._id}</div>
                    ...
                    <div className="perf-stats">
                      <div className="perf-value" style={{ color: "#10b981" }}>
                        +₹{Math.round(med.profit).toLocaleString("en-IN")}
                      </div>
                      <div className="perf-subtext">
                        Sales: ₹{med.sales.toLocaleString("en-IN")}
                      </div>
